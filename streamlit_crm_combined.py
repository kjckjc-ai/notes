...
tab1, tab2, tab3, tab4 = st.tabs(["Add Note", "Contacts", "Notes Search & Export", "Actions"])
...
with tab2:
    ...
    if not contacts.empty:
        ...
        new_tags = st.text_input("Tags", value=row["tags"])
        new_notes = st.text_area("Notes", value=row["notes"], height=100)
    ...
    if not df.empty:
        ...
        if st.button("Download Markdown"):
            ...
            for _, row in df.iterrows():
                ...
                md += f"**Tags:** {row['tags']}\n"
                md += f"{row['body']}\n\n"
            st.download_button("Download Markdown", md, "meeting_notes.md")
    else:
