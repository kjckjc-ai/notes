...
with tab1:
    ...
    title = st.text_input("Meeting Title", key="note_title")
    date_val = st.date_input("Date", datetime.today(), key="note_date")
    contacts_df = get_contacts()
    contact_name_to_id = dict(zip(contacts_df['name'], contacts_df['id'])) if not contacts_df.empty else {}
    contact_selection = st.selectbox("Select Contact", ["None"] + list(contact_name_to_id.keys()), key="note_contact")
    contact_id = contact_name_to_id.get(contact_selection) if contact_selection != "None" else None
    participants = st.text_input("Other Participants", key="note_participants")
    tags = st.text_input("Tags (comma-separated)", key="note_tags")
    body = st.text_area("Meeting Notes", height=200, key="note_body")
...
with tab2:
    ...
    tags = st.text_input("Tags (comma-separated)", key="contact_tags")
    notes = st.text_area("Notes", height=100, key="contact_notes")
    if st.button("Save Contact", key="save_contact_btn"):
        if name:
